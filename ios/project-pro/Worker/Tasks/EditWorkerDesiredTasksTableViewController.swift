//
//  EditWorkerDesiredTasksTableViewController.swift
//  project-pro
//
...
class EditWorkerDesiredTasksTableViewController: UITableViewController {
	
	// task id's start at 1
	var cells = [Task]()
	...
	@IBAction func doneButtonDidPress(_ sender: Any) {
		let url = URL(string: Endpoint.editWorkerDesires)!
		// create the session object
		let session = URLSession.shared
		// now create the URLRequest object using the url object
		var request = URLRequest(url: url)
				...
				if let json = try JSONSerialization.jsonObject(with: data, options: .mutableContainers) as? [String: Any] {
					...
					if
						...
						if status {
							...
							print("POST /editWorkerDesires Error")
						}
					}
				}
			} catch let error {
