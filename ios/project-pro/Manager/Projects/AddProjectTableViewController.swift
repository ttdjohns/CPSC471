...
class AddProjectTableViewController: UITableViewController {
	...
	@IBAction func doneButtonDidPress(_ sender: Any) {
		let url = URL(string: Endpoint.addProject)!
		...
		do {
			...
			if let description = self.descriptionTextField.text {
				parameters["Project_description"] = description
			}
			if let accessLevel = User.accessLevel, accessLevel != "2", let teamID = self.teamTextField.text{
				parameters["Team_ID"] = Int(teamID)!
			}
	...
	override func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
		if let accessLevel = User.accessLevel, accessLevel == "2", indexPath.row == 2 {
			return 0
		}
