//
//  EditCauseTableViewController.swift
//  project-pro
//
...
class EditCauseTableViewController: UITableViewController {
	...
	override func viewWillAppear(_ animated: Bool) {
		...
		nameTextField.text = "\(cause.name)"
		descriptionTextField.text = "\(cause.description)"
	}
	...
	@IBAction func doneButtonDidPress(_ sender: Any) {
		let url = URL(string: Endpoint.editCause)!
		...
		do {
			...
			if let name = self.nameTextField.text {
				parameters["Cause_name"] = name
			}
			if let description = self.descriptionTextField.text {
				parameters["Cause_description"] = description
			}
			parameters["Projects"] = cause.projects
	...
	override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
		if indexPath.section == 1 {
			let url = URL(string: Endpoint.removeCause)!
			// create the session object
			let session = URLSession.shared
			// now create the URLRequest object using the url object
			var request = URLRequest(url: url)
